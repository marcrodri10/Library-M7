<div class="card">
    <div class="card-body">
        <h5 class="card-title">Month</h5>
        <div class="price-value" data-currency="$ USD" data-currency-simple="USD">
            <p class="price-number">$<span class="price-integer">1/mo</span></p>
        </div>
        <div class="pricing-note">1 month subscription</div>
        <ul class="list-group check-list">
            <li class="list-group-item">
                <svg class="bi" width="16" height="16" fill="currentColor">
                    <use xlink:href="#check-icon"></use>
                </svg>
                Full Access to the Catalog
            </li>
            <li class="list-group-item">
                <svg class="bi" width="16" height="16" fill="currentColor">
                    <use xlink:href="#check-icon"></use>
                </svg>
                Read Books Anytime
            </li>
            <li class="list-group-item">
                <svg class="bi" width="16" height="16" fill="currentColor">
                    <use xlink:href="#check-icon"></use>
                </svg>
                Explore New Titles
            </li>
            <li class="list-group-item">
                <svg class="bi" width="16" height="16" fill="currentColor">
                    <use xlink:href="#check-icon"></use>
                </svg>
                Ad-Free Experience
            </li>
            <li class="b-flex-center-center-row">
                <button class="cssbuttons-io-button w-100" type="submit" name="subscription" value="month">
                    Subscribe
                    <div class="icon b-flex-center-center">
                        <svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path d="M0 0h24v24H0z" fill="none"></path>
                            <path d="M16.172 11l-5.364-5.364 1.414-1.414L20 12l-7.778 7.778-1.414-1.414L16.172 13H4v-2z" fill="currentColor"></path>
                        </svg>
                    </div>
                </button>
            </li>
        </ul>


    </div>
</div>